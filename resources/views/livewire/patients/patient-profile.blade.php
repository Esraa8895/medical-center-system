<div>
    <!-- Back -->
    <div class="mb-6">
        <a href="/patients" class="text-sm font-medium flex items-center gap-1 w-fit" style="color:#511269;">
    ...
    <div class="rounded-2xl p-6 mb-6 text-white" style="background: linear-gradient(135deg, #511269, #7B2D8B);">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <div class="flex gap-4 mt-1 text-purple-200 text-sm flex-wrap">
                    @if($patient->phone)
        ...
        <div class="grid grid-cols-4 gap-4 mt-6">
            ...
            <div class="rounded-xl p-3 text-center" style="background: rgba(255,255,255,0.15);">
                @php $remaining = $totalAmount - $totalPaid; @endphp
                <p class="text-2xl font-bold">{{ number_format(max(0, $remaining)) }}</p>
                <p class="text-xs text-purple-200 mt-0.5">المتبقي</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div class="border border-gray-100 rounded-lg p-3 mb-3 last:mb-0">
                ...
                <div class="mt-2">
                    <div class="flex justify-between text-xs text-gray-500 mb-1">
                        ...
                        <span>{{ number_format($plan->expected_total) }} ل.س</span>
                    </div>
            ...
            @empty
            <p class="text-gray-400 text-sm text-center py-6">لا يوجد خطط علاج</p>
            @endforelse
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div class="flex justify-between items-center mb-3 p-3 rounded-lg" style="background: #f5f0fa;">
                ...
                <span class="font-bold" style="color: #511269;">{{ number_format($totalPaid) }} ل.س</span>
            </div>
            @forelse($payments as $payment)
            <div class="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                <div>
                    <p class="text-sm font-medium text-gray-800">{{ number_format($payment->amount) }} <span class="text-xs text-gray-400">ل.س</span></p>
                    <p class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($payment->created_at)->format('Y-m-d H:i') }}</p>
                </div>
                @if($payment->notes)
                <span class="text-xs text-gray-400 max-w-24 text-left truncate">{{ $payment->notes }}</span>
                @endif
            </div>
            @empty
            <p class="text-gray-400 text-sm text-center py-6">لا يوجد مدفوعات</p>
            @endforelse
        </div>
        ...
        <table class="w-full text-right text-sm">
            <thead>
                <tr class="border-b border-gray-100">
                    ...
                    <th class="pb-3 font-semibold text-gray-600">المتبقي</th>
                    <th class="pb-3 font-semibold text-gray-600">التاريخ</th>
                </tr>
            </thead>
            <tbody>
                @forelse($visits as $visit)
                @php $rem = $visit->total_amount - $visit->paid_cost; @endphp
                <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                    <td class="py-3 text-gray-400 font-mono text-xs">{{ $visit->id }}</td>
                    <td class="py-3 text-gray-700">{{ $visit->doctor_name }}</td>
                    <td class="py-3 font-semibold text-gray-800">{{ number_format($visit->total_amount) }}</td>
                    <td class="py-3 text-green-600 font-semibold">{{ number_format($visit->paid_cost) }}</td>
                    <td class="py-3">
                        @if($rem > 0)
                        <span class="px-2 py-0.5 rounded-full text-xs bg-red-100 text-red-600">{{ number_format($rem) }}</span>
                        @else
                        <span class="px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-600">مسدد ✅</span>
                        @endif
                    </td>
                    <td class="py-3 text-gray-400 text-xs font-mono">{{ \Carbon\Carbon::parse($visit->created_at)->format('Y-m-d') }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="py-8 text-center text-gray-400">لا يوجد زيارات</td>
                </tr>
                @endforelse
            </tbody>
        </table>
